...
func _ready() -> void:
	...
	GameManager.on_faith.connect(self._on_faith)
	GameManager.on_game_over.connect(self._on_game_over_notified)
	GameManager.go_interstage.connect(self._in_scene_dialogue)
	GameManager.note_hit.connect(self._on_note_hit)
