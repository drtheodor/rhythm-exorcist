...
func _ready():
	...
	var microseconds_per_tick: float = midi_resource.tempo / midi_resource.division;
	var speed_scale: float = 1
	
	var notes_on: Array[Variant] = []
			...
			if event['type'] == 'note':
				...
				if event.subtype == MIDI_MESSAGE_NOTE_ON:
					last_note = { 
						...
						"note": event.note, 
						"time": time, 
						"duration": (time - last_note.time if last_note else 0.1)
					}
	...
	for note_data: Variant in notes_on:
